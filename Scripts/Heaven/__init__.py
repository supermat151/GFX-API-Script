...
# Copyright (C) 2012-2013, Unigine Corp. All rights reserved.
#
# File:    __init__.py
# Desc:    Heaven automation core
# Version: 1.03
# Authors: Anna Chaplinskaya <anna@unigine.com>
#          Denis Shergin <binstream@unigine.com>
#
# This file is part of the Heaven benchmark Advanced/Pro
# http://unigine.com/products/heaven/pro/

import os
import platform
import string

engine_project = 'Heaven'
...
def run(\
	...
	""" Starts Heaven benchmark with custom parameters.
	...
	tessellation: DISABLED, MODERATE, NORMAL, EXTREME (case-insensitive)
	...
	engine_define = '-extern_define RELEASE,HEAVEN_ADV,AUTOMATION'
	
	engine_define += ',QUALITY_' + quality.upper()
	engine_define += ',TESSELLATION_' + tessellation.upper()
	
	if frame_number > -1: engine_define = engine_define + ',FRAME'
	
	if log_caption == '': engine_log_caption = 'FPS,API,Resolution,AA,Tessellation,Quality,Video,CPU'
	else: engine_log_caption = log_caption
	
	if log_format == '': engine_log_format = '$F,$A,$v,$m,$quality,$tessellation,$g,$c'
	else: engine_log_format = log_format
	
	engine_log_format = str.replace(engine_log_format,'$quality',quality)
	engine_log_format = str.replace(engine_log_format,'$tessellation',tessellation)
	
	if system == 'Linux' or system == 'Darwin':
		engine_log_format = string.replace(engine_log_format,'$','\$')
